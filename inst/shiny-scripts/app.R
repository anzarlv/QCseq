...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      # Output: Tabset w/ welcome, QC tables/plots, and references
      tabsetPanel(
        type = "tabs",
...
head(qc_results)</code></pre>
      ...
      <ol>
        ...
        <li>
          Explore the tabs in order:
          <ul>
            <li><strong>QCcompute</strong> – Per-cell QC metrics from
                <code>QCcompute()</code> (n_genes, n_transcripts,
                percent_mit, percent_ribo).</li>
            <li><strong>QCscore</strong> – QC metrics plus the unified QC
                rank score <code>qc_score_rank</code> from
                <code>QCscore()</code>.</li>
            <li><strong>QC summary</strong> – Plain language dataset-level
                summary generated by <code>QCsummary()</code> using your
                chosen thresholds.</li>
            ...
            <li><strong>References</strong> – Key literature and software
                resources underlying the QCseq workflow and this app.</li>
          </ul>
        </li>
      </ol>
